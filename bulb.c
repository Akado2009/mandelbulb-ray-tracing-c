...
int main() {
    ...
    Vector camera_position = {.x = 1.25f, .y = 0.0f, .z = 1.0f};
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            ...
            if (!isinf(hit.distance)) {
                ...
                color.r = (unsigned char)clip((color_strength * 120.0f) * brightness, 0.0f, 255.0f);
                color.g = (unsigned char)clip((color_strength * 240.0f) * brightness, 0.0f, 255.0f);
                color.b = (unsigned char)clip((color_strength * 78.0f) * brightness, 0.0f, 255.0f);
            }
