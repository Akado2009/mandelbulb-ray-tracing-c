...
int main() {
    ...
    Vector camera_position = {.x = 8.0f, .y = 0.0f, .z = 7.0f};
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            ...
            if (!isinf(hit.distance)) {
                ...
                color.r = (unsigned char)clip((color_strength * 19.0f) * brightness, 0.0f, 255.0f);;
                color.g = (unsigned char)clip((color_strength * 11.0f) * brightness, 0.0f, 255.0f);
                color.b = (unsigned char)clip((color_strength * 133.0f) * brightness, 0.0f, 255.0f);
            }
    ...
    BMP_WriteFile(bmp, "result_box4.bmp");
    return 0;
}